...
from optim_factory_ats import create_optimizer, LayerDecayValueAssigner

# from datasets import build_dataset, build_transform
from engine_entropy_loss import train_one_epoch, evaluate

from utils import NativeScalerWithGradNormCount as NativeScaler
import utils
from losses_fast import ConvNextDistillDiffPruningLoss, DistillDiffPruningLoss_dynamic
from samplers import RASampler
# from models.dyconvnext import ConvNeXt_Teacher, AdaConvNeXt
# from models.lvvit_conv_tp import LVViTDiffPruning, LVViT_Teacher
from model_ip_specific_fast import VisionTransformerDiffPruning, VisionTransformerTeacher
from lvvit_ip_specific_fast import LVViTDiffPruning, LVViT_Teacher
# from models.model_ip_specific_ats import VisionTransformerDiffPruning, VisionTransformerTeacher

## Jul 7: model_granular_fast is the original
...
# python main.py --output_dir logs/dynamicvit_deit-s --model deit-s --input_size 224 --batch_size 256 --epochs 30 --base_rate 0.7 --lr 1e-3 --warmup_epochs 5 --auto_resume False


# python main.py --output_dir logs/dynamicvit_deit-s --model deit-s --input_size 224 --batch_size 256 --epochs 30 --base_rate 0.7 --warmup_epochs 5 --ratio_weight 5.0 --distill_weight 0.5 --clf_weight 1.0 --lr 0.001 --auto_resume False
def get_args_parser():
    ...
    parser.add_argument('--keep_ratio', default="0.7_0.5_0.35",
                        help='keep ratio list')
    # '--nargs-int-type', nargs = '+', type = int
    # * Mixup params
    ...
    parser.add_argument('--sharing', default='',
    ...
    parser.add_argument('--clf_weight', type=float, default='2.0')
...
def main(args):
    ...
    sharing_list = args.sharing.split("_")

    if sharing_list[0] is not '0':
        layer_configs = []
        print(f'length {len(sharing_list)} {sharing_list}')
        for i in range(len(sharing_list)):
            layer_configs.append(int(sharing_list[i]))
    else:
        layer_configs = []


    # sharing = args.sharing.split("-")
    #
    ...
    if args.data_set == 'IMNET':
        ...
        train_dataset = '/gpfs/gibbs/project/panda/shared/imagenet_ffcv/train_jpg.beton'  # imagenet100_ffcv/train_500_0.50_90.ffcv'
        val_dataset = '/gpfs/gibbs/project/panda/shared/imagenet_ffcv/val.beton'  # imagenet100_ffcv/val_500_0.50_90.ffcv'
        data_loader_train = img_ffcv.create_train_loader(train_dataset, num_workers, batch_size,
    ...
    if args.model == 'convnext-t':
        ...
        PRUNING_LOC = [4, 8, 12]
        # layer_configs = sharing
        KEEP_RATE = keep_list_float #[SPARSE_RATIO[0], SPARSE_RATIO[0] ** 2, SPARSE_RATIO[0] ** 3]
        ...
        pretrained_shared = torch.load('../../pretrained/lvvit_s-26M-224-83.3.pth.tar', map_location='cpu')
        ...
        model = VisionTransformerDiffPruning(
            patch_size=16, embed_dim=384, depth=12, num_heads=args.num_heads, mlp_ratio=4, qkv_bias=True,
            pruning_loc=PRUNING_LOC, token_ratio=KEEP_RATE, distill=True, layer_configs=layer_configs
        )
        # pretrained_shared = torch.load('./pretrained/dynamic-vit_384_r0.7.pth',
        #                                map_location='cpu')  # pretrained_with_sharing[(0, 1)] pretrained_with_sharing[(4, 5, 6)]
        pretrained_shared = torch.load('../pretrained/deit_small_patch16_224-cd65a155.pth', map_location='cpu')
        ...
        PRUNING_LOC = [3,6,9]
        KEEP_RATE = keep_list_float
        print('token_ratio =', KEEP_RATE, 'at layer', PRUNING_LOC)
        model = VisionTransformerDiffPruning(
            patch_size=16, embed_dim=256, depth=12, num_heads=4, mlp_ratio=4, qkv_bias=True,
            pruning_loc=PRUNING_LOC, token_ratio=KEEP_RATE, distill=True, layer_configs=layer_configs
            )
        pretrained_shared = torch.load('../pretrained/dynamic-vit_256_r0.7.pth', map_location='cpu')
        pretrained = torch.load('../pretrained/dynamic-vit_256_r0.7.pth', map_location='cpu')
        teacher_model = VisionTransformerTeacher(
            patch_size=16, embed_dim=256, depth=12, num_heads=4, mlp_ratio=4, qkv_bias=True)
    ...
    if 'convnext' in args.model or 'deit' in args.model or 'swin' in args.model:
        ...
        pretrained_shared = pretrained_shared['model']
    ...
    if 'convnext' in args.model:
        ...
        criterion = DistillDiffPruningLoss_dynamic(
            teacher_model, criterion, clf_weight=1.0, keep_ratio=KEEP_RATE, mse_token=False, ratio_weight=2.0,
            distill_weight=0.5
        )
    elif 'deit' in args.model:
        criterion = DistillDiffPruningLoss_dynamic(
            teacher_model, criterion, clf_weight=args.clf_weight, keep_ratio=KEEP_RATE, mse_token=True,
            ratio_weight=args.ratio_weight, distill_weight=args.distill_weight
        )
    ...
    print("LR = %.8f" % args.lr)
    print("Batch size (total_batch_size) = %d" % total_batch_size)
    print("Update frequent = %d" % args.update_freq)

    print("Number of training examples = %d (Warning: CORRECT THIS)" % (len(data_loader_train)))
    ...
    optimizer = create_optimizer(
        args, model_without_ddp, skip_list=None, filter_bias_and_bn=True,
    ...
    if args.eval:
        ...
        print(
            f"Accuracy of the network on {len(data_loader_val) * args.batch_size} test images: {test_stats['acc1']:.5f}%")
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            # test_stats = evaluate(data_loader_val, model, device, epoch, reuse_pattern=[],
            #                       use_amp=args.use_amp)
            print(
                f"Accuracy of the model on the {len(data_loader_val) * args.batch_size} test images: {test_stats['acc1']:.1f}%")
            ...
            if args.model_ema and args.model_ema_eval:
                test_stats_ema = evaluate(data_loader_val, model_ema.ema, device, use_amp=True)
                # test_stats_ema = evaluate(data_loader_val, model_ema.ema, device, epoch, reuse_pattern=[],
                #                       use_amp=args.use_amp)

                print(
                    f"Accuracy of the model EMA on {len(data_loader_val) * args.batch_size} test images: {test_stats_ema['acc1']:.1f}%")
