...
def get_parameter_groups(model, weight_decay=1e-5, skip_list=(), get_num_layer=None, get_layer_scale=None, bone_lr_scale=0.01):
    ...
    for name, param in model.named_parameters():
        print(name, ("transform" in name))
        if not param.requires_grad:
            continue  # frozen weights
        elif 'cls_token' in name or 'pos_embed' in name:
            continue # frozen weights
        elif 'fast' in name or 'transform' in name or 'mse' in name or 'fastmlp' in name:
        # elif 'predictor' in name:
            if 'transform' not in name:
            ...
            if "norm" not in name:
                ...
                group_name = "transform_param"
                this_weight_decay = 0.
            elif "norm" in name:
                group_name = "norm_params"
                this_weight_decay = 0
        ...
        if group_name not in parameter_group_names:
            ...
            if group_name is 'decay':
                scale = bone_lr_scale
                fix_step = -1
            elif group_name is "norm_params":
                scale = bone_lr_scale
                fix_step = -1
            elif group_name is "transform_params":
                scale = bone_lr_scale
                fix_step = -1
            elif group_name is "qkv_params":
                scale = bone_lr_scale
                fix_step = -1
