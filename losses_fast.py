...
class ConvNextDistillDiffPruningLoss(torch.nn.Module):
    ...
    def __init__(self, teacher_model, base_criterion: torch.nn.Module, ratio_weight=10.0, distill_weight=0.5,
                 keep_ratio=[0.9, 0.7, 0.5], clf_weight=0, mse_token=False, print_mode=True, swin_token=False):
        super().__init__()
    ...
    def forward(self, inputs, outputs, labels):
        ...
        loss = self.clf_weight * cls_loss + self.ratio_weight * pred_loss / len(
            out_pred_score) + self.distill_weight * cls_kl_loss + self.distill_weight * token_kl_loss
        loss_part = []
        ...
        if self.print_mode:
            ...
            if self.count == 100:
                print(
                    'loss info: cls_loss=%.4f, ratio_loss=%.4f, cls_kl=%.4f, token_kl=%.4f, layer_mse=%.4f, feat_kl=%.4f' % (
                    self.cls_loss / 100, self.ratio_loss / 100, self.cls_distill_loss / 100,
                    self.token_distill_loss / 100, self.layer_mse_loss / 100, self.feat_distill_loss / 100))
                self.count = 0
                self.cls_loss = 0
                self.ratio_loss = 0
                self.cls_distill_loss = 0
...
class DistillDiffPruningLoss_dynamic(torch.nn.Module):
    ...
    def forward(self, inputs, outputs, labels):
        ...
        #     if i == 0:
        #         loss_attn = F.kl_div(
        #         F.log_softmax(attn_list[i], dim=-1),
        #         F.log_softmax(attn_list_t[i], dim=-1),
        #         reduction='batchmean',
        #         log_target=True)
        #     else:
        #         loss_attn += F.kl_div(
        ...
        #         log_target=True)
        #
        # if len(attn_list) is not 0:
        #     loss_attn /= len(attn_list)
        ...
        # if len(attn_list) is not 0:
        loss = self.clf_weight * cls_loss + self.distill_weight * cls_kl_loss + self.distill_weight * token_kl_loss #+ self.distill_weight * loss_attn
        # print(self.clf_weight * cls_loss, self.distill_weight * cls_kl_loss, self.distill_weight * token_kl_loss)
        # else:
        #     loss = self.clf_weight * cls_loss + self.distill_weight * cls_kl_loss + self.distill_weight * token_kl_loss #+ self.distill_weight * loss_attn
        ...
        if self.print_mode:
            ...
            self.count += 1
            if self.count == 100:
                print('loss info: cls_loss=%.4f, cls_kl=%.4f, token_kl=%.4f' % (
                self.cls_loss / 100, self.cls_distill_loss / 100, self.token_distill_loss / 100))
                self.count = 0
                self.cls_loss = 0
                # self.ratio_loss = 0
                self.cls_distill_loss = 0
