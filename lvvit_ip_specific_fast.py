...
class Block(nn.Module):
    ...
    def forward(self, x, policy=None, padding_mask=None, reuse=False):
        if reuse == False:
...
class LVViTDiffPruning(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 p_emb='4_2', head_dim=None,
                 skip_lam=1.0, order=None, mix_token=False, return_dense=False, pruning_loc=None, token_ratio=None,
                 distill=False, viz_mode=False, layer_configs=[]):
        super().__init__()
        ...
        self.layer_confgs = layer_configs
        if order is None:
        ...
        for i, blk in enumerate(self.blocks):
            if i in self.pruning_loc:
                ...
                if self.training:
                    ...
                    x = blk(x, policy=policy, reuse= i in self.layer_confgs)
                    ...
                    x = blk(x, reuse=i in self.layer_confgs)
                p_count += 1
            else:
                if self.training:
                    x = blk(x, policy, reuse= i in self.layer_confgs)
                else:
                    x = blk(x, reuse= i in self.layer_confgs)
